...
// ==================== 后端 API 代理 ====================
const BACKEND_SERVER_URL = process.env.BACKEND_SERVER_URL || 'http://localhost:3001';

app.use('/api', async (req, res, next) => {
	try {
		const targetUrl = new URL(req.originalUrl, BACKEND_SERVER_URL);
		...
		broadcastBackendEvent(req.method, req.originalUrl, text);
		res.send(text);
	} catch (error) {
		console.error(`❌ 后端代理失败: ${req.method} ${req.originalUrl}`, error.message);
		res.status(502).json({
			code: 502,
			success: false,
			message: `无法连接到后端服务 ${BACKEND_SERVER_URL}`,
			error: error.message,
...
function broadcastBackendEvent(method, path, responseText) {
	...
	if (!data || payload.success === false || payload.code >= 400) return;

	if (path.startsWith('/api/admin/judges')) {
		broadcast('judges-updated', data);
		return;
	}

	if (path.startsWith('/api/admin/debate-flow/control')) {
		broadcast('debate-flow-control', data);
		return;
	}

	if (path.startsWith('/api/admin/debate-flow')) {
		broadcast('debate-flow-updated', data);
		return;
	}

	if (path.includes('/live/start') || path.includes('/live/stop') || path.includes('/live/control') || path.includes('/live/setup-and-start')) {
		broadcast('live-status-changed', data);
		return;
	}

	if (path.includes('/votes') || path.includes('/user-vote')) {
		broadcast('votes-updated', data);
	}
}
// ==================== 后端 API 代理结束 ====================
